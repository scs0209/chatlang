...
import { parseCodex } from "@chatlang/parse-codex";
import { parseCursor } from "@chatlang/parse-cursor";
  ...
  <div class="frame">
    ...
    <main>
      ...
      <section class="pane">
        ...
        <div class="code" id="output">// parsers wait on golden fixtures — see README Assignment</div>
        <div class="actions">
...
function renderHighlighted(text: string, tokens: Token[]): string {
  ...
  for (const tok of sorted) {
    ...
    if (tok.start > cursor) {
      parts.push(escapeHtml(text.slice(cursor, tok.start)));
    }
    const slice = text.slice(tok.start, tok.end);
    parts.push(`<span class="tok-${tok.type}">${escapeHtml(slice)}</span>`);
    cursor = tok.end;
  }
...
function render(): void {
  ...
  if (!text.trim()) {
    output.innerHTML = escapeHtml(
      "// paste a session · parsers unlock after fixtures",
    );
    output.dataset.raw = "";
